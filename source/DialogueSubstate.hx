...
class DialogueSubstate extends FlxSubState
{
	...
	override public function new(song:String, player1:String, onFinish:Void->Void, delay:Float = 0.4, suffix = "", BGColor:FlxColor = FlxColor.TRANSPARENT)
	{
		super(BGColor);
		this.song = song;
		this.player = player1;
	...
	override function create()
	{
		...
		dialogue = Json.parse(StringTools.trim(File.getContent(Paths.json(song + '/' + "dialogue/" + player + suffix))));
	...
	function preloadStuff()
	{
			...
			if (speaker != null)
			{
				...
				for (face in speakers.get(speaker))
				{
					if (FileSystem.exists("assets/images/dialogue/portraits/" + speaker + "/" + face + ".funk"))
					{
						...
						faces[speaker][face] = Paths.getImageFunk("dialogue/portraits/" + speaker + "/" + face);
					}
					else
					{
						// trace("NO PORTRAIT OF " + face + " FOUND FOR " + speaker);
						faces[speaker][face] = Paths.getImageFunk("dialogue/portraits/default");
					}
				}
			}
		}
		...
		for (boxType in boxTypes)
		{
			if (boxType != null)
			{
				boxes[boxType] = Paths.getImageFunk("dialogue/" + boxType);
			}
		}
	}
	...
	function spitDialogue()
	{
		...
		textBox.loadGraphic(boxes[boxType]);
		...
		var theFont = (curDiag.font == null ? "knewave" : curDiag.font);
	...
	function playMusic(newSong:String)
	{
		...
		if (newSong != "none")
		{
			musicStream = new AudioStreamThing(Paths.music("dialogue/" + newSong));
