 ...
 * @author Zaphod
 */
class FlxNestedSkewSprite extends FlxSkewedSprite
{
	/**
	...
	public var relativeColorTransform:ColorTransform = new ColorTransform();

	/**
	 * All FlxNestedSprites in this list.
	 */
	public var children(default, null):Array<FlxNestedSkewSprite> = [];
	...
	public var ignoreScreenBounds:Bool = false;
	 ...
	 */
	public function add(Child:FlxNestedSkewSprite):FlxNestedSkewSprite
	{
		if (children.contains(Child))
			return Child;

		children.insert(addIndex, Child);
		addIndex++;
	...
	public function killWithChildren()
	{
	...
	public function setRelativeColorTransform(redMultiplier:Float = 1.0, greenMultiplier:Float = 1.0, blueMultiplier:Float = 1.0, alphaMultiplier:Float = 1.0,
			redOffset:Int = 0, greenOffset:Int = 0, blueOffset:Int = 0, alphaOffset:Int = 0):Void
	{
		FlxColorTransformUtil.setMultipliers(relativeColorTransform, redMultiplier, greenMultiplier, blueMultiplier, alphaMultiplier);
		FlxColorTransformUtil.setOffsets(relativeColorTransform, redOffset, greenOffset, blueOffset, alphaOffset);
	}
	...
	function refreshDimensions()
	{
		if (children.length == 0)
		{
			var c00x = x;
			var c00y = y;
			var c01x = x + frameWidth;
			var c01y = y;
			var c10x = x;
			var c10y = y + frameHeight;
			var c11x = x + frameWidth;
			var c11y = y + frameHeight;

			var maxX:Float = Math.NEGATIVE_INFINITY;
			var maxY:Float = Math.NEGATIVE_INFINITY;
			for (coord in [[c00x, c00y], [c01x, c01y], [c10x, c10y], [c11x, c11y]])
			{
				coord[0] = coord[0] * transformMatrix.a + coord[1] * transformMatrix.c + transformMatrix.tx;
				coord[1] = coord[0] * transformMatrix.b + coord[1] * transformMatrix.d + transformMatrix.ty;
	 ...
	 */
	public function remove(Child:FlxNestedSkewSprite):FlxNestedSkewSprite
	{
	 ...
	 */
	public function removeAt(Index:Int = 0):FlxNestedSkewSprite
	{
	...
	// 	}
	// }
	var shouldPassDownAngle:Bool = true;
	var shouldPassDownScale:Bool = true;
	...
	override public function update(elapsed:Float):Void
	{
		...
		for (child in children)
		{
			...
			if (child.active && child.exists)
			{
				...
				if (shouldPassDownAngle)
					child.angle = angle + child.relativeAngle;
				if (shouldPassDownScale)
				{
	...
	override public function draw():Void
	{
		...
		{
			if (!camera.visible || !camera.exists || (!ignoreScreenBounds && !isOnScreen(camera)))
				continue;
		...
		for (child in children)
		{
			...
			{
				child.ignoreScreenBounds = ignoreScreenBounds;
				child.draw();
			}
		}
	}
	...
	override function drawComplex(camera:FlxCamera):Void
	{
		...
		for (child in children)
		{
			child.setColorTransform(child.relativeColorTransform.redMultiplier * colorTransform.redMultiplier,
				child.relativeColorTransform.greenMultiplier * colorTransform.greenMultiplier,
				child.relativeColorTransform.blueMultiplier * colorTransform.blueMultiplier,
				child.relativeColorTransform.alphaMultiplier * colorTransform.alphaMultiplier,
				Std.int(child.relativeColorTransform.redOffset)
				+ Std.int(colorTransform.redOffset),
				Std.int(child.relativeColorTransform.greenOffset)
				+ Std.int(colorTransform.greenOffset),
				Std.int(child.relativeColorTransform.blueOffset)
				+ Std.int(colorTransform.blueOffset),
				Std.int(child.relativeColorTransform.alphaOffset)
				+ Std.int(colorTransform.alphaOffset));
		}
